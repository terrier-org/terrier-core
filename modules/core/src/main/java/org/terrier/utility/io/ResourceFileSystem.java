...
public class ResourceFileSystem implements FileSystem
{
	...
	{
		InputStream rtr = ResourceFileSystem.class.getResourceAsStream(filename.replaceFirst("resource:", ""));
		if (rtr == null)
			throw new FileNotFoundException("Did not find " + filename + " in resources");
		return rtr;
	}
	...
	{
		boolean there = false;
		InputStream is = ResourceFileSystem.class.getResourceAsStream(filename.replaceFirst("resource:", ""));
		if (is != null)
		{
			there = true;
			try {
